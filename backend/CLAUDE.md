...
- Multi-tenancy: every tenant-owned table (`employee`, `repo`, `policy_doc`, and any new one you add) has a non-nullable `org_id` FK into `organization.id`. `org_id` must **never** come from the request body/query/path — it only ever comes from `CurrentOrgId` (`api/dependency/tenancy.py`), which resolves the Clerk org id out of the verified session token and auto-provisions the `Organization` row on first sight (there's no Clerk org webhook receiver yet). Every DAO method that reads/writes a tenant-owned row must take `org_id` as an explicit argument and filter/set on it — see `EmployeeDAO`/`RepoDAO` (`get_by_id_for_org`, `list_for_org`) for the pattern. When you implement one of the still-`NotImplementedError` domains (`skill_graph`, `rag`, `quiz`, `chat`/archaeology, `expert_routing`, `dashboard`), thread `org_id` through the same way — most of them hang off `repo_id`/`employee_id`, which are themselves already org-scoped, so a query that joins through those tables and also filters by `org_id` at the top is the safe default.

...
async def chunk_and_embed(repo_id: str, services: ServiceContainer = Depends(get_service_container)):
    """Kick off chunking + embedding of a repo's code (PRD §6.1)."""
    return await services.rag.chunk_and_embed(repo_id)


@router.post("/retrieve", response_model=RetrieveResponse)
async def retrieve(repo_id: str, payload: RetrieveRequest, services: ServiceContainer = Depends(get_service_container)):

...
async def get_expertise_scores(repo_id: str, services: ServiceContainer = Depends(get_service_container)):
    """Per-file expertise scores derived from git history (PRD §6.2)."""
    return await services.skill_graph.compute_expertise_scores(repo_id)


@router.get("/bus-factor", response_model=list[BusFactorResponse])
async def get_bus_factor(repo_id: str, services: ServiceContainer = Depends(get_service_container)):
    """Bus-factor risk per file derived from the expertise scores (PRD §6.2)."""
    return await services.skill_graph.compute_bus_factor(repo_id)
